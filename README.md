...
$$
a^{(k)}_{ij} = a^{(k - 1)}_{ij} - \mu_{ij}a^{(k - 1)}_{ij}, \quad
b^{(k)}_{ij} = b^{(k - 1)}_{ij} - \mu_{ij}b^{(k - 1)}_{ij}, \quad
i = k + 1, \dots m
$$

Для избежания сильного росто коэффициентов системы и связанных с этим ошибок нельзя допускать появления больший множителей $\mu_{ij}$.

В метода гарантируется, что $|\mu_{ij}| \leq 1 \quad \forall \quad k = 1, 2, \dots, m - 1 $ и $i = k + 1, \dots m$.
...
$$
\delta(x^{\ast}) \leq f(m) cond_E(A) \epsilon_M
$$
