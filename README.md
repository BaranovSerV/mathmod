...
Найти решение системы $Ax = b$ с точностью $\epsilon$. Это означает, что нужно найти вектор $x^{\ast}$ такой, что $\left\| \overline{x} - x^{*} \right\| \leq \epsilon$, где $\left\| \cdot \right\|$ — одна из норм (единичная, евклидова, бесконечная).
